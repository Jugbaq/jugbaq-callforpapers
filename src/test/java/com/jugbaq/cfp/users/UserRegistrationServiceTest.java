...
class UserRegistrationServiceTest {
    ...
    void should_register_speaker_with_encoded_password() {
        ...
        User found =
                userRepository.findByEmailIgnoreCase("nuevo.speaker@test.com").orElseThrow();
        assertThat(found.getPasswordHash()).isNotEqualTo("miPassword123");
        assertThat(passwordEncoder.matches("miPassword123", found.getPasswordHash()))
                .isTrue();
        assertThat(found.getSpeakerProfile()).isNotNull();
        assertThat(found.getTenantRoles()).hasSize(1);
        assertThat(found.hasRole(
                        tenantRepository.findBySlug("jugbaq").orElseThrow().getId(), TenantRole.SPEAKER))
                .isTrue();
    }
